 ...
 * through `createBooking`, `issueInvoiceFor` and `postEntry` — the same code
 * the office app runs,
...
export const run = platformMutation({
  ...
  handler: async (ctx, args) => {
    ...
    /*
     * THROUGH `insertSite`, which is the primitive siteConfigs.ts exports for
     * exactly this — its own comment names seeding as a caller. Inserting
     * here instead would need this file added to the sites-writer allowlist,
     * and the allowlist is for writers that genuinely cannot use the
     * primitive. This one can.
     */
    ...
    const customerId = await ctx.db.insert("customers", {
      ...
      phone: "+27825559911",
      /*
       * @example.com, RESERVED BY RFC 2606 AND DELIBERATE.
       *
       * The customer needs an address or the confirmation has nowhere to go
       * and the seeded outbox shows a hole rather than the state a real one
       * would be in. It must never be a person's: `example.com` cannot be
       * registered and does not resolve, so this stays safe even if the send
    ...
    /*
     * THROUGH `createBooking`, NOT AN INSERT OF ITS OWN.
     *
     * The first version inserted the row directly, which meant deleting the
     * guard that keeps `bookings.ts` the only writer of `startsAt` — and that
     * guard is what stops a second writer forgetting `messageRevision` and
     * silently suppressing a rescheduled customer's confirmation. An
     * exception granted to a seeder is an exception that widens.
     *
     * It is also the better seed. A booking that skipped the real path has no
     * overlap check, no revision and no queued confirmation, so every screen
     * built against it is being judged on data that does not behave like
     * production — which is the one thing a seeder must not do.
     *
     * The confirmation it queues is real and goes to an @example.com address
     * that cannot resolve. That is the point: the outbox then shows what a
     * live one shows.
     */
    ...
    const invoice = await issueInvoiceFor(ctx, {
      ...
      now: now - day(10),
      // Required rather than optional — an invoice always names who raised it,
      // and a seeded one is raised by whoever ran the seeder.
      actorUserId: actor,
    });
    ...
    const second = await issueInvoiceFor(ctx, {
      clientId,
